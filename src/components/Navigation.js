...
const Navigation = props => {
    ...
    return (
        <nav className="fixed-top ml-4 mt-2">
            <div className="nav">
                <Link to={`/${homePage}/${props.charName}`} className="nav-item btn btn-danger mx-1">Home</Link>
                <Link to={`/${aboutMePage}/${props.charName}`} className="nav-item btn btn-danger mx-1">About me</Link>
